...
const GeneratePaymentLinkModal = ({ show, onClose, onConfirm, lead, role }) => {
  ...
  const isLockedForRole = (lead?.status === 'PAID' || lead?.status === 'EMI' || lead?.paymentOrderId) && !isAdmin;
        ...
        <div className="p-4 overflow-auto custom-scroll">
          {isLockedForRole ? (
            <div className="text-center py-5 animate-fade-in">
              <div className="p-3 bg-success bg-opacity-10 text-success rounded-circle d-inline-flex mb-4 shadow-sm border border-success border-opacity-10">
                <ShieldCheck size={48} />
              </div>
              <h5 className="fw-black text-dark mb-2 text-uppercase tracking-widest">Protocol Secured</h5>
              <p className="text-muted small fw-bold mb-0 opacity-50 px-4">
                This student node has already been initialized with a payment link or has completed its financial transmission. 
                Redundant link generation is blocked to preserve ledger integrity.
              </p>
              <button className="btn btn-outline-secondary mt-5 rounded-pill fw-bold px-4" onClick={onClose}>
                Close Terminal
              </button>
            </div>
          ) : (
